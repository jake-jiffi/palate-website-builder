...
- **Explicit jury review or `/palate-website-builder:jury`:** use [the jury entry point](commands/jury.md), including for an existing non-Palate codebase. This takes precedence over the legacy build route for that request; preserve the current stack. Jury uses the same shared four-round allowance as automatic escalation, with early stopping. These are internal design judgements, separate from the optional grading instrument.
