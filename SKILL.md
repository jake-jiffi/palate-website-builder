...
A. **Scaffold + Explore + Compose** (one phase, one project, several NAMED substages). The
   stage order is the spine every gate hangs off: DIVERGE -> CONVERGE -> EXPLORE -> per-variant
   + post-compose VISUAL LOOP -> mandatory VERIFY -> gates. Each substage that produces evidence
   writes its block to `build-manifest.json` (`references/build-manifest.md`).
...
- **Reference = craft + buildability; concept layer = novelty.** The MCP tells you what is *good and buildable* (donor craft, `astro_recipe` feasibility); the concept layer plus the DIVERGE/CONVERGE spine is where *originality* comes from. Never let a donor's safe default substitute for an original concept; never let an original concept exceed what the stack can build (the buildability oracle decomposes it back to buildable mechanics, forthcoming in `references/reference-library-usage.md`).
...
- **Run the verifier before a build is done (mandatory, in a fresh context).** Spawn the `palate-verifier` subagent (`agents/palate-verifier.md`) in an isolated context after Compose and before any preview hand-off; do not reuse the building agent's context, the verifier is the independent check. It runs every gate in one pass: MCP depth, uniqueness across the variants (`scripts/gate-uniqueness.mjs`), novelty across builds (`scripts/gate-novelty.mjs`: a safe-only converge, a near-repeat of a recent build, or a recurring display face), anti-default/slop lint, provenance mixing (no donor > half the sections; compose from at least three), the visual loop + fixed rubric (`references/visual-rubric.md`), and verify-is-real-astro. It writes `verify-report.json` (computed evidence, not narration) and returns one pass/fail report; fix the named sections and re-verify, capped at 2-3 cycles then escalate. The deterministic gates are authoritative. The build is not done until `verify-report.json` exists with `verdict: pass`.
