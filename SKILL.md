...
A. **Scaffold + Explore + Compose** (one phase, one project, several NAMED substages). The
   ...
   2. **A.2 DIVERGE** (the concept spine, sample wide). From the Story Engine's one true thing + the spine, sample N=6-8 candidate concepts in one pass (Verbalized Sampling): each self-tags its `conventionality` (0..1, how typical it is for the category), the `lens` it came from (span >=2 lenses), and a cross-domain `analogical_seed`. Carry the lower-typicality tail forward. Writes `manifest.diverge`. The full doctrine lands in `references/story-engine.md` (forthcoming in the spine work); the substage and its manifest block are live now.
   3. **A.3 CONVERGE** (score, then narrow). Score each diverged candidate on two separate axes (originality + craft-feasibility), combine, and advance the best 1-2 into Explore. Run `scripts/gate-novelty.mjs` as a pre-check so a safe-only converge is caught (forthcoming `gate-novelty.mjs`; fail-open until present). Writes `manifest.converge`.
