...
A. **Scaffold + Explore + Compose** (one phase, one project, several NAMED substages). The
   ...
   9. **A.9 VISUAL LOOP + VERIFY (mandatory, fresh context).** Spawn `palate-verifier` as a subagent (isolated context) to run the visual loop (`scripts/screenshot-build.mjs`) over the composed page and every gate in one pass. The build is not done until `verify-report.json` exists with `verdict: pass`. The Stop hook enforces this via `gate-done.sh` (warn by default, hard-block under `PALATE_GATE_STRICT=1`) - forthcoming in the gate work; the visual-loop + verifier wiring is live now.
