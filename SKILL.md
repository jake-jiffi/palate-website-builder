...
- **The visual rubric gate (judge the render, then loop).** A build is not done until its RENDERED page clears the 6-axis rubric. After Compose (and for each Explore variant before it is shown), screenshot the page desktop + mobile and score the six axes (`references/critique-discipline.md` section 2b) against the pixels, plus the perceptual floors. The bar we hold to: every axis >=4, no floor violation, no placeholder imagery. Any axis at 3 or below, or a floor violation, means revise the named section, re-render, and re-score, looping until it clears. Judge the artefact, not the code.
- **Run the verifier before a build is done.** Spawn the `palate-verifier` subagent (`agents/palate-verifier.md`) to run every gate in one isolated pass: MCP depth, uniqueness across the variants (`scripts/gate-uniqueness.mjs`), anti-default/slop lint, provenance mixing (no donor > half the sections; compose from at least three), the visual rubric gate, and verify-is-real-astro. It returns one pass/fail report; fix the named sections and re-verify, capped at 3 cycles then escalate. The deterministic gates are authoritative.
