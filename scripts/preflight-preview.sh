...
# token UNLESS JIFFI_PREVIEW_DELIVERY=local (a local dev-server preview, which
# needs no cloud creds at all).
set -euo pipefail
fail() { echo "PREVIEW PREFLIGHT FAIL: $1" >&2; echo "  remediation: $2" >&2; exit 1; }
ok() { echo "  ok: $1"; }
echo "jiffi-website-builder preview preflight"

# Environment + working dir (proven writable by detect-environment)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
eval "$(bash "$SCRIPT_DIR/detect-environment.sh")" || fail "could not find a writable working directory" "set JIFFI_WORK_DIR to a writable path"
[ -n "${WORK_ROOT:-}" ] || fail "no working directory resolved" "set JIFFI_WORK_DIR"
...
# can review + leave Toolbar Comments). Set JIFFI_PREVIEW_DELIVERY=local (or pass
# --local-preview) for a local dev-server preview that needs no cloud creds.
DELIVERY="${JIFFI_PREVIEW_DELIVERY:-vercel}"
