...
if command -v claude >/dev/null 2>&1; then
  if [ -n "$TOKEN" ]; then
    claude mcp add palate --url https://mcp.palatemcp.com --header "Authorization: Bearer $TOKEN" >/dev/null 2>&1 \
    ...
    say "    claude mcp add palate --url https://mcp.palatemcp.com --header \"Authorization: Bearer <plt_live_ token>\""
