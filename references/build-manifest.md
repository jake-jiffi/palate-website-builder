...
```jsonc
{
  ...
  "explore": null,      // W1 labels + the board record: { ran, boards:[{ id, rung, donor }], shown_at, canvas:{ url } | { skipped:true, reason }, shown:[{ id, name, donor_slug, position }], picks:[{ surface, variant_id, rung, position, picked_at }], question_round:{ motion, mix, cms, answered_at }, second_passes:int, cta, notes:[{ surface, variant_id, note, at }], edits:[...], proof:{ url, verified_at }, donor_row:{ skipped:true, reason } | { skipped:false, count, slugs:[] }, board_judgements:[{ id, donor, rung, consistent, run_token, judged_at }] } - persisted to builds.log.json by palate-stop.mjs. boards-render.mjs writes boards + shown_at + shown + donor_row (donor_row records a skipped donor row with its reason, so a canvas drawn without the references its boards reproduce cannot be mistaken for one whose donors were never recorded), gate-board-judge.mjs writes board_judgements (one entry per registered board, the LOWER of the two orders, recorded even when the gate refuses, which is what gate-explore.mjs reads to block a shown board that was never compared with its donor or was read clearly_worse), palate-pick.mjs writes the rest through manifest-merge.mjs and there is a flag for every field of it (--hero/--section, --intensity, --cta, --note, --canvas-url | --canvas-skipped, --answer, --second-pass, --proof), so none of it is ever hand-edited; time to pick is picked_at - shown_at, which is the only number this stage has ever produced. A board is its ARTBOARD (src/lib/variants.ts carries artboard:"B<rung>.dc.html", required; there is no route), so canvas says what happened to the published canvas and gate-explore.mjs blocks a shown build that recorded neither a url nor a reason, and gate-done.sh blocks a build that recorded picks without a complete question_round (palate-pick.mjs --answer motion=... --answer mix=... --answer cms=...)
