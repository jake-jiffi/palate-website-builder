...
Open the supporting `refs_get_screenshot` result and available motion evidence before claiming you observed a technique. A successful call containing truncated image text is not a viewed screenshot; obtain a usable image result or state the limit. Reject blank or challenge-page captures and refine ambiguous page names. A search result or written recipe can suggest a mechanism, but label it as suggested when no usable motion evidence was observed.
...
Show a convincing slice of that idea in the first opening and useful exploration section, then share it. Keep the source's main service or product families discoverable through navigation or exploration. Carry the visual idea through substantive content to the primary action, rather than dropping into unrelated generic sections. Alternatives need different composition, section logic or interaction, beyond colour and type changes. Do not invent product claims or before/after evidence to support an effect.
