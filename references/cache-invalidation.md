...
ships whatever is there, and five local commands rebuild `dist/` without `PUBLIC_SITE_ENV`
(`verify-scaffold.sh`, `verify-is-real-astro.sh`, `serve-preview.sh`, `phantom-utility-check.mjs`
and `boards-render.mjs`). An unbaked build noindexes itself, and the contact endpoint now
refuses the post-deploy smoke header on it rather than honouring it, so the round trip skips
with a reason instead of a live site quietly discarding enquiries. `npm run deploy` bakes
`production` and then deploys.
