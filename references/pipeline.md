...
finalise rows). Every build is STATIC from Phase A with three declared on-demand
exceptions; a CMS is opt-in and most builds have none (see `build-stages.md`).
...
| A.4 EXPLORE *(default: ON)* | (draw one artboard per rung at `.palate/explore/seed/B<rung>.dc.html`, register them in `src/lib/variants.ts`, `boards-render.mjs`, publish the canvas, `PUBLIC_EXPLORE_MODE=true`) | PAUSE - client picks on the canvas or `/explore` | one direction board per rung, each the whole home page in that direction, on a ladder from restrained to bold; the canvas published (or declined with a reason) and `/explore` shipped with the stills. See `explore-stage.md`. |
