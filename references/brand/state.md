...
`.jiffi-brand-state.json` lives in the brand repo working dir and makes runs resumable. Created by `state-init.sh`, mutated atomically by `state-update.sh`, queried by `state-resume.sh`.
...
When jiffi-website-builder invokes this skill in Phase 0 and the run is interrupted, the website-builder's own state records the brand repo path. On the website-builder's resume, it checks for `.jiffi-brand-state.json` in the brand repo and resumes this skill first before continuing to Phase A.
